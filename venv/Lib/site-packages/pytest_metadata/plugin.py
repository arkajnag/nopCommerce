...
def pytest_configure(config):
    ...
    if hasattr(config, "subordinateoutput"):
        config.subordinateoutput["metadata"] = config._metadata
...
def pytest_testnodedown(node):
    # note that any metadata from remote subordinates will be replaced with the
    # environment from the final subordinate to quit
    if hasattr(node, "subordinateoutput"):
        node.config._metadata.update(node.subordinateoutput["metadata"])
